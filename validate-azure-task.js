...
async function validateTaskJson() {
  ...
  try {
    ...
    // Required fields
    const requiredFields = ['id', 'name', 'friendlyName', 'description', 'version', 'inputs', 'execution'];
...
async function validateTaskExecution() {
  ...
  try {
    ...
    const { stdout, stderr } = await execAsync('cd dist && node -e "require(\'./azure-pipeline.js\')"', {
...
async function testWithMockInputs() {
  ...
  try {
    ...
    const { stdout, stderr } = await execAsync('cd dist && node azure-pipeline.js', {
      env: mockEnv,
    ...
    if (error.message.includes('timeout')) {
      logSuccess('Task appears to be working (timed out during Claude execution)');
      return true;
...
async function validateAzureCliIntegration() {
  ...
  try {
    ...
    if (stdout.includes('azure-devops')) {
      ...
      logWarning('Azure DevOps extension not found. Install with: az extension add --name azure-devops');
    }
...
async function generateUploadInstructions() {
  ...
  console.log('   - Or use: tfx extension publish --manifest-globs vss-extension.json');
  console.log('');
